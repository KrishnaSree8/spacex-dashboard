...
const getStatusLabel = (launch_success) => {
	if (launch_success === null) {
		...
		return (
			<Button size="tiny" color="yellow" style={{background: '#16BA49', color: 'white', border: '1px solid green'}}>
				Successful
			</Button>
		);
	} else {
		return (
			<Button size="tiny" style={{background: 'red', color: 'white', border: '1px solid red'}}>
				Failed
			</Button>
		);
	}
